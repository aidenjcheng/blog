...
const isCardImage = (child: ReactNode) =>
  isValidElement(child) && (child.type === CardImage || (child.type as { displayName?: string })?.displayName === "CardImage");
...
const Card = forwardRef<HTMLDivElement, CardProps>(
  (
    ...
    const cardContext = useMemo<CardContextValue>(() => ({ emphasized, orientation, clickable, hasImage }), [emphasized, orientation, clickable, hasImage]);
    ...
    if (inlineImage) {
      ...
      body = (
        <>
          {image}
          <div className={cn("flex min-w-0 flex-1 flex-col justify-center gap-2", compact ? "py-2.5 pr-3" : "py-3.5 pr-4")}>{rest}</div>
        </>
      );
    }
      ...
      <CardContext.Provider value={cardContext}>
        <div
          ...
          className={cn(
            ...
            dismissible &&
              isInline &&
              (dismissOnHover ? "[&:focus-within_[data-slot=card-header]]:pr-10 [&:hover_[data-slot=card-header]]:pr-10" : "[&_[data-slot=card-header]]:pr-10"),
            tileShape,
        ...
        >
          ...
          {selected && <span aria-hidden className={cn("-z-10 pointer-events-none absolute inset-0 bg-active", shape.container)} />}
          ...
          {showBottom && <span aria-hidden className="-z-10 pointer-events-none absolute inset-x-0 bottom-0 h-px bg-border/60" />}
          {showRight && (
            <span aria-hidden className={cn("-z-10 pointer-events-none absolute top-0 right-0 w-px bg-border/60", showBottom ? "bottom-px" : "bottom-0")} />
          )}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        inlineImage ? "min-w-0" : orientation === "inline" ? cn("min-w-0 flex-1", compact ? "py-2.5" : "py-3.5") : compact ? "px-3 pt-3" : "px-4 pt-4",
        className,
      )}
      {...props}
    />
...
const CardTitle = forwardRef<HTMLSpanElement, HTMLAttributes<HTMLSpanElement>>(({ className, children, ...props }, ref) => {
  ...
  return (
    <span
      ...
      className={cn("inline-grid grid-cols-[minmax(0,1fr)] leading-snug", compact ? "text-[13px]" : "text-[14px]", className)}
      {...props}
    >
      <span
        className={cn("invisible col-start-1 row-start-1 min-w-0 overflow-hidden text-ellipsis", trim)}
      ...
      <span
        className={cn("col-start-1 row-start-1 min-w-0 overflow-hidden text-ellipsis text-foreground transition-[font-variation-settings] duration-80", trim)}
        style={{
...
const CardDescription = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <p
      ...
      className={cn("text-muted-foreground leading-normal", compact ? "text-[13px]" : "text-[14px]", className)}
      {...props}
    />
  );
});
...
const CardAction = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div ref={ref} data-slot="card-action" className={cn("relative z-30 col-start-2 row-span-2 row-start-1 self-start justify-self-end", className)} {...props} />
));
...
const CardContent = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  const sizeClasses = useSize();
  const compact = sizeClasses.variant === "compact";
  return <div ref={ref} data-slot="card-content" className={cn(orientation === "inline" ? "" : compact ? "px-3 pt-2.5" : "px-4 pt-3", className)} {...props} />;
});
...
function CardMedia({ logo, logoAlt, icon: Icon, size = 22, className }: CardMediaProps) {
  ...
  if (logo) {
    ...
    return (
      <span data-slot="card-media" className={cn("inline-flex shrink-0 items-center gap-1.5", wrap)}>
        {logos.map((src, i) => (
          <span key={src} className="inline-flex items-center gap-1.5">
            {i > 0 && <span aria-hidden className="h-px w-2 bg-border" />}
            <Image src={src} alt={logoAlt ?? ""} width={size} height={size} className={cn("object-contain", shape.bg)} style={{ width: size, height: size }} />
          </span>
        ))}
      </span>
    );
  ...
  if (Icon) {
    ...
    return (
      <span data-slot="card-media" className={cn("inline-flex size-8 shrink-0 items-center justify-center bg-hover", shape.bg, wrap)}>
...
function CardImage({ src, alt, className }: CardImageProps) {
  ...
  return (
    <Image
      ...
      className={cn("rounded-[2px] object-cover", orientation === "inline" ? "size-40 shrink-0" : "aspect-[16/9] w-full", className)}
    />
  );
}
...
const CardEyebrow = forwardRef<HTMLSpanElement, HTMLAttributes<HTMLSpanElement>>(({ className, ...props }, ref) => {
  const compact = useSize().variant === "compact";
  return (
    <span
      ref={ref}
      data-slot="card-eyebrow"
      className={cn(compact ? "text-[11px]" : "text-[12px]", "text-muted-foreground uppercase tracking-wide", className)}
      style={{ fontVariationSettings: fontWeights.semibold }}
      {...props}
    />
  );
...
function CardFeature({ icon: Icon, title, description }: CardFeatureProps) {
  ...
  return (
    <div data-slot="card-feature" className={cn("flex items-start", sizeClasses.gap)}>
      {Icon && <Icon size={sizeClasses.icon} strokeWidth={1.5} className="mt-0.5 shrink-0 text-muted-foreground" />}
      <div className="flex min-w-0 flex-col gap-0.5">
        <span className={cn("text-foreground [text-box:trim-both_cap_alphabetic]", sizeClasses.text)} style={{ fontVariationSettings: fontWeights.medium }}>
          {title}
        </span>
        {description && <span className={cn("text-muted-foreground leading-relaxed", compact ? "text-[11px]" : "text-[12px]")}>{description}</span>}
      </div>
    </div>
  );
}
...
function CardButton({ children, onClick, href, variant = "ghost", icon: Icon, iconPosition, external = false, disabled = false }: CardButtonProps) {
  const shape = useShape();
  ...
  const glyph = Icon ? (
    <Icon size={compact ? 12 : 14} strokeWidth={1.5} className="shrink-0 transition-[stroke-width] duration-80 group-hover/action:stroke-[2]" />
  ) : null;
  const externalGlyph = external ? (
    <ArrowRight size={13} strokeWidth={1.5} className="-rotate-45 shrink-0 transition-[stroke-width] duration-80 group-hover/action:stroke-[2]" />
  ) : null;
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled} className={classes} style={{ fontVariationSettings: fontWeights.medium }}>
      {inner}
    </button>
  );
}
